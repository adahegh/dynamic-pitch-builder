...
export function PitchBuilder() {
  ...
  const handleSubmit = async () => {
    if (!userInput.trim() && !uploadedFile && !productDescription.trim()) {
      toast.error("Please enter a website URL, upload a PDF file, or provide a product description.");
      return;
    }
    ...
    let userMessage = "";
    const parts = [];
    if (userInput.trim()) parts.push(`Website URL: ${userInput}`);
    if (productDescription.trim()) parts.push(`Product Description: ${productDescription}`);
    if (uploadedFile) parts.push(`uploaded file: ${uploadedFile.name}`);
    
    userMessage = parts.join(" and ");
    ...
    try {
      ...
      const templateInfo: ProductInfo = {
        website: uploadedFile ? '' : userInput.trim(),
        source: uploadedFile ? 'pdf' : (userInput.trim() ? 'website' : 'manual'),
                ...
                <div className="grid gap-6">
                  ...
                  <div className="bg-white p-4 rounded-lg border">
                    ...
                    <div className="space-y-3 text-sm">
                      {productInfo.source === 'pdf' ? (
                        <div><strong>Source:</strong> <span className="text-blue-600">PDF Document ({productInfo.fileName})</span></div>
                      ) : productInfo.website ? (
                        <div><strong>Product Website:</strong> <span className="text-blue-600">{productInfo.website}</span></div>
                      ) : (
                        <div><strong>Source:</strong> <span className="text-blue-600">Manual Entry</span></div>
                      )}
          ...
          {step === 'edit-strategy' && editingStrategy && (
            <div className="w-1/2 flex flex-col">
              <div className="bg-purple-50 p-6 rounded-lg border flex-1 overflow-y-auto">
                ...
                <div className="space-y-6">
                  {/* Cold Call Starters Section */}
                  {editingStrategy.coldCallStarters && (
                    <div>
                      <h4 className="font-semibold text-lg mb-3 text-green-700">Cold Call Starters</h4>
                      <div className="space-y-3">
                        {editingStrategy.coldCallStarters.map((starter, index) => (
                          <Card key={index} className="p-4 bg-white">
                            ...
                            <Textarea
                              value={starter}
                              onChange={(e) => {
                                const newStarters = [...editingStrategy.coldCallStarters];
                                newStarters[index] = e.target.value;
                                setEditingStrategy({...editingStrategy, coldCallStarters: newStarters});
                              }}
                              placeholder={`Enter cold call starter ${index + 1}`}
                              rows={2}
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setEditingStrategy({
                              ...editingStrategy,
                              coldCallStarters: [...(editingStrategy.coldCallStarters || []), ""]
                            });
                          }}
                          className="text-green-600 border-green-300 hover:bg-green-50"
                        >
                          Add Cold Call Starter
                        </Button>
                      </div>
                    </div>
                  )}
                ...
                <div className="bg-blue-50 p-6 rounded-lg border animate-fade-in">
                  ...
                  <div className="grid gap-6">
                    ...
                    <div className="bg-white p-4 rounded-lg border">
                      <h4 className="font-semibold text-blue-700 mb-3">Product Understanding</h4>
                      <div className="space-y-3">
                        {editingInfo.source === 'pdf' && (
                          <div className="bg-gray-50 p-3 rounded-lg border">
                            <div className="text-sm text-gray-600 mb-2">
                              <strong>Source:</strong> PDF Document ({editingInfo.fileName})
                            </div>
                            <label className="block text-sm font-medium mb-1">Product Website (optional):</label>
                            <Input
                              value={editingInfo.website}
                              onChange={(e) => setEditingInfo({...editingInfo, website: e.target.value})}
                              placeholder="Enter website URL if available (e.g., https://example.com)"
                              className="h-10 text-base"
                            />
                          </div>
                        )}
                        {editingInfo.source === 'website' && (
                          <div className="bg-gray-50 p-3 rounded-lg border">
                    ...
                    {editingEmailCadence.map((cadenceStep, index) => (
                      <div key={index} className="bg-white p-4 rounded-lg border">
                        <div className="flex justify-between items-start mb-3">
                          <h4 className="font-semibold text-orange-700">Step {index + 1}</h4>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => {
                              const newCadence = editingEmailCadence.filter((_, i) => i !== index);
                ...
                <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-lg border animate-fade-in">
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-lg border">
                      ...
                      <div className="space-y-6">
                        {/* Cold Call Starters */}
                        {pitchStrategy.coldCallStarters && pitchStrategy.coldCallStarters.length > 0 && (
                          <div>
                            <h5 className="font-medium text-gray-700 mb-3">Cold Call Starters:</h5>
                            <div className="space-y-3">
                              {pitchStrategy.coldCallStarters.map((starter, index) => (
                                <div key={index} className="bg-green-50 p-4 rounded-lg flex items-start">
              ...
              <div className="border-t p-4">
                <div className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div className="text-center text-gray-600">and/or</div>
                    ...
                    <div className="space-y-2">
                      <Textarea
                        ...
                        onChange={(e) => {
                          ...
                          updatePromptStrength(e.target.value, 'description', 'description');
                        }}
                        className="w-full min-h-[80px] text-base"
                      />
                      {productDescription.trim() && (
                        <div className="flex justify-end">
                          <PromptStrengthIndicator strength={promptStrengths['description'] || 0} fieldKey="description" />
                        </div>
                      )}
                    </div>
                  ...
                  <Button 
                    ...
                    disabled={!userInput.trim() && !uploadedFile && !productDescription.trim()}
                    className="w-full"
                  >
