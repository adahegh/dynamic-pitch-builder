...
export function PitchBuilder() {
  ...
  const handleFileUpload = async (file: File | string) => {
    setIsLoading(true);
    addMessage('user', typeof file === 'string' ? `Website URL: ${file}` : `Uploaded file: ${file.name}`);
    
    // Simulate processing delay
    setTimeout(() => {
      const mockProductInfo: ProductInfo = {
        website: typeof file === 'string' ? file : 'uploaded-document.pdf',
  ...
  const handleInputSubmit = () => {
    ...
    if (userInput.startsWith('http')) {
      handleFileUpload(userInput);
    } else {
      toast.error("Please enter a valid website URL or upload a PDF file.");
    }
    setUserInput('');
  };
